...
class ModelCheckoutOrder extends Model {	
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		foreach ($this->model_account_customer->getCustomOptions(1) as $option){
			if($option['email_display']){
				$value = $this->model_account_customer->getCustomerOrderOptions($option['option_id'],'personal',$order_id);			
		...
		foreach ($this->model_account_customer->getCustomOptions(2) as $option){			
			...
			foreach ($this->model_account_customer->getCustomOptions(2) as $option){
				if($option['email_display']){
					$value = $this->model_account_customer->getCustomerOrderOptions($option['option_id'],'payment',$order_id);			
					$replace= array_merge($replace, array($option['identifier'] => $value));}
		...
		foreach ($this->model_account_customer->getCustomOptions(2) as $option){			
			...
			foreach ($this->model_account_customer->getCustomOptions(2) as $option){
				if($option['email_display']){
					$value = $this->model_account_customer->getCustomerOrderOptions($option['option_id'],'shipping',$order_id);			
					$replace= array_merge($replace, array($option['identifier'] => $value));}
				else $replace= array_merge($replace, array($option['identifier'] => ''));
			}
