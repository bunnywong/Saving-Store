<?php
class ModelCheckoutXOrder extends Model {

	public function createInvoiceNo($order_id) {
		$order_info = $this->getOrder($order_id);

		if ($order_info && !$order_info['invoice_no']) {
			$myInvoicePrefix = 'DS' . substr(date("Ym"), 2, 6);

			$query = $this->db->query("SELECT MAX(invoice_no) AS invoice_no FROM `" . DB_PREFIX . "order` WHERE invoice_prefix = '".$myInvoicePrefix."'");
	...
	public function addOrder($data) {
		...
		if((int)$data['customer_id']==0){
			$this->addCustomerOrderOptionData($data,$order_id,0,$data['shipping_method']);
		}else{
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if ($order_info['customer_id']) {
				...
				foreach ($results as $result) {
					if ($this->model_account_xcustomer->isBanIp($result['ip'])) {
						$status = true;
          ...
          $template->data['comment'] = '';
			...
			foreach ($this->model_account_xcustomer->getCustomOptions(1) as $option){
				...
				$stringP .= '<b>'.$option['name'].': </b>{'.$option['identifier'].'}'."\n";
			}
			$find = array();
			foreach ($this->model_account_xcustomer->getCustomOptions(1) as $option){
				if($option['email_display'])
			...
			foreach ($this->model_account_xcustomer->getCustomOptions(1) as $option){
				if($option['email_display']){
					$value = $this->model_account_xcustomer->getCustomerOrderOptions($option['option_id'],'personal',$order_id);
			...
			$find = array(
				...
				foreach ($this->model_account_xcustomer->getCustomOptions(2) as $option){
					if($option['email_display']){
						$value = $this->model_account_xcustomer->getCustomerOrderOptions($option['option_id'],'payment',$order_id);
						$replace= array_merge($replace, array($option['identifier'] => $value));}
				...
				foreach ($this->model_account_xcustomer->getCustomOptions(2) as $option){
					if($option['email_display']){
						$value = $this->model_account_xcustomer->getCustomerOrderOptions($option['option_id'],'shipping',$order_id);
						$replace= array_merge($replace, array($option['identifier'] => $value));}
						else $replace= array_merge($replace, array($option['identifier'] => ''));
				}
	...
	public function addCustomerOrderOptionData($data,$order_id,$customer_id,$hasShipping){
		$this->load->model('account/xcustomer');
		foreach ($this->model_account_xcustomer->getCustomOptions(0) as $option){
			$this->db->query("INSERT INTO " . DB_PREFIX . "xcustom_customer_order_option
