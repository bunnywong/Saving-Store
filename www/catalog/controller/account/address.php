...
//      			'zone'      => $result['zone'],
      			'zone'      => substr($result['zone'], 5),		// My Script
				'zone_code' => $result['zone_code'],
      			'country'   => $result['country']
			);
    		foreach ($this->model_account_customer->getCustomOptions(2) as $option){
				if($option['list_display']){
					$value = $this->model_account_customer->getCustomerOptionsA((int)$this->customer->getId(),$option['option_id'],$option['type'],$result['address_id']);
					$replace= array_merge($replace, array($option['identifier'] => $value));
				}else
    				$replace= array_merge($replace, array($option['identifier'] => ''));
			}
      		$this->data['addresses'][] = array(
  	...
  	foreach ($this->model_account_customer->getCustomOptions(2) as $option) {
			if ($option['type'] != 'checkbox' && isset($this->request->post['option'.$option['option_id']])) {
  			...
  			elseif ($option['type'] != 'checkbox' && !empty($address_info)) {
				$this->data['optionV'.$option['option_id']] = $this->model_account_customer->getCustomerAddressOptions($this->customer->getId(),$option['option_id'],null,$this->request->get['address_id']);
			}
			...
			if (isset($this->request->post['optionV'.$option['option_id'].'C'.$option_value['option_value_id']])) {
				...
				$this->data['optionV_O'.$option['option_id'].'C'.$option_value['option_value_id']] = $this->model_account_customer->getCustomerAddressOptions($this->customer->getId(),$option['option_id'],$option_value['option_value_id'],$this->request->get['address_id']);
			}
			else{
