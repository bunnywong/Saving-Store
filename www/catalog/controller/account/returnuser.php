...
class ControllerAccountReturnuser extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$message  .= 'http://savingstore.com.hk/index.php?route=account/returnuser&email='.base64_encode($email).'&pwd='.base64_encode($pwd). "\n\n\n";
...
//			$message .= '<img src="http://greenmap.hk/image/data/logo.png" style="max-width: 180px; height: auto;">';	// todo IMG
		...
		if ( isset( $this->request->get['email']) && isset( $this->request->get['pwd']) ) {
			...
			// Pwd 		: YnVubnlzb24=				(bunnyson)
				...
				// Decode
			$this->data['error_warning']   = base64_decode($email);
			$this->data['error_warning']   .= ' / ';
			$this->data['error_warning']   .= base64_decode($pwd);
			...
			$this->session->data['success'] =  '你的帳戶已被啟動';	// Success msg
//			$this->redirect($this->url->link('account/login', '', 'SSL'));
		}
		// ----- ----- ----- ----- -----
