<?php
class ControllerCheckoutXRegister extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/xregister.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/checkout/xregister.tpl';
		} else {
			$this->template = 'default/template/checkout/xregister.tpl';
		}
	...
	public function validate() {
		...
		if (!$json) {
			...
			foreach ($this->model_account_xcustomer->getCustomOptions() as $option) {
				...
				if ($this->model_account_xcustomer->getTotalCustomersByEmail($this->request->post['email'])) {
		...
		if (!$json) {
			$this->model_account_xcustomer->addCustomer($this->request->post,$modData,$isActive);
			...
			$isAddress=$this->model_account_xaddress->getAddresses();
			if(empty($isAddress)){
