	...
	public function validate() {
		...
		foreach ($this->model_account_customer->getCustomOptions() as $option) {
			...
			$this->load->model('account/customer');
				foreach ($this->model_account_customer->getCustomOptions(0) as $option) {
					if($option['type']=='select' || $option['type']=='radio' || $option['type']=='checkbox')
						if($option['section']==1)				
							$this->session->data['xtensions']['personal'][$option['identifier']]=$this->model_account_customer->getGuestOptionsA($option['option_id'],$option['type'],$this->session->data['guest']['payment']['option'.$option['option_id']]);
						else
							$this->session->data['xtensions']['payment'][$option['identifier']]=$this->model_account_customer->getGuestOptionsA($option['option_id'],$option['type'],$this->session->data['guest']['payment']['option'.$option['option_id']]);	
					else
						if($option['section']==1) 
			...
			foreach ($this->model_account_customer->getCustomOptions(2) as $option) {
				...
				foreach ($this->model_account_customer->getCustomOptions(2) as $option) {
					if($option['type']=='select' || $option['type']=='radio' || $option['type']=='checkbox')						
						$this->session->data['xtensions']['shipping'][$option['identifier']]=$this->model_account_customer->getGuestOptionsA($option['option_id'],$option['type'],$this->session->data['guest']['shipping']['option'.$option['option_id']]);	
					else
