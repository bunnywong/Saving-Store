...
$('<?php ((!$isActive['enablemod'] || $isActive['enablemod'] && $modData['country_show_edit'])?'select':'input');?>[name=\'country_id\']').bind('change', function() {
	$.ajax({
		...
		dataType: 'json',
		beforeSend: function() {
			$('<?php ((!$isActive['enablemod'] || $isActive['enablemod'] && $modData['country_show_edit'])?'select':'input');?>[name=\'country_id\']').after('<span class="wait">&nbsp;<img src="catalog/view/theme/default/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
			$('.wait').remove();
		},
