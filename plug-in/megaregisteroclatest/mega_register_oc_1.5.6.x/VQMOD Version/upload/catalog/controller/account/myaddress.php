...
class ControllerAccountMyAddress extends Controller {
		...
		foreach ($results as $result) {
			...
			$find = array(
				...
				'zone_code' => $result['zone_code'],
      			...
      			foreach ($this->model_account_xcustomer->getCustomOptions(2) as $option){
      				if($option['list_display']){
      					$value = $this->model_account_xcustomer->getCustomerOptionsA((int)$this->customer->getId(),$option['option_id'],$option['type'],$result['address_id']);
        		...
        		'update'     => $this->url->link('account/myaddress/update', 'address_id=' . $result['address_id'], 'SSL'),
				'delete'     => $this->url->link('account/myaddress/delete', 'address_id=' . $result['address_id'], 'SSL')
      			);
		}

		$this->data['insert'] = $this->url->link('account/myaddress/insert', '', 'SSL');
	...
	protected function getForm() {
		...
		$this->data['title_country'] = $this->language->get('title_country');
		$this->data['title_zone'] = $this->language->get('title_zone');
		...
		if (!isset($this->request->get['address_id'])) {
			$this->data['action'] = $this->url->link('account/myaddress/insert', '', 'SSL');
		} else {
			$this->data['action'] = $this->url->link('account/myaddress/update', 'address_id=' . $this->request->get['address_id'], 'SSL');
		}

		if (isset($this->request->get['address_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$address_info = $this->model_account_xaddress->getAddress($this->request->get['address_id']);
		}
		...
		} elseif (!$modData1['country_show_edit'] && $modData1['def_country']) {
		...
		} elseif (!$modData1['state_show_edit'] && $modData1['def_state']) {
		...
		foreach ($this->model_account_xcustomer->getCustomOptions(2) as $option) {
			...
			elseif ($option['type'] != 'checkbox' && !empty($address_info)) {
				$this->data['optionV'.$option['option_id']] = $this->model_account_xcustomer->getCustomerAddressOptions($this->customer->getId(),$option['option_id'],null,$this->request->get['address_id']);
			}
			...
			if($option['type'] == 'checkbox'){
				foreach ($option['option_value'] as $option_value) {
					if (isset($this->request->post['optionV'.$option['option_id'].'C'.$option_value['option_value_id']])) {
						...
						$this->data['optionV_O'.$option['option_id'].'C'.$option_value['option_value_id']] = $this->model_account_xcustomer->getCustomerAddressOptions($this->customer->getId(),$option['option_id'],$option_value['option_value_id'],$this->request->get['address_id']);
					}
					else{
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/myaddress_form.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/myaddress_form.tpl';
		} else {
			$this->template = 'default/template/account/myaddress_form.tpl';
		}
	...
	protected function validateForm() {
		...
		foreach ($this->model_account_xcustomer->getCustomOptions(2) as $option) {
			...
			if ((!$isActive || ($isActive && $modData['address1_req_edit'] && $modData['address1_show_edit'])) && ((utf8_strlen($this->request->post['address_1']) < 3) || (utf8_strlen($this->request->post['address_1']) > 128))) {
				$this->error['address_1'] = $this->language->get('error_address_1');
			}

			if ((!$isActive || ($isActive && $modData['city_req_edit'] && $modData['city_show_edit'])) && ((utf8_strlen($this->request->post['city']) < 2) || (utf8_strlen($this->request->post['city']) > 128))) {
				$this->error['city'] = $this->language->get('error_city');
			}

			$this->load->model('localisation/country');

			if(!$isActive || ($isActive && $modData['pin_req_edit'] && $modData['pin_show_edit'] && $modData['country_show_edit']))
			$country_info = $this->model_localisation_country->getCountry($this->request->post['country_id']);

			if ((!$isActive || ($isActive && $modData['pin_req_edit'] && $modData['pin_show_edit'] && $modData['country_show_edit'])) && $country_info) {
			...
			if ((!$isActive || ($isActive && $modData['country_req_edit'] && $modData['country_show_edit'])) && $this->request->post['country_id'] == '') {
				$this->error['country'] = $this->language->get('error_country');
			}

			if ((!$isActive || ($isActive && $modData['state_req_edit'] && $modData['state_show_edit'] && $modData['country_show_edit'] )) && (!isset($this->request->post['zone_id']) || $this->request->post['zone_id'] == '')) {
	...
	protected function validateDelete() {
		if ($this->model_account_xaddress->getTotalAddresses() == 1) {
